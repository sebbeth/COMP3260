/**
 * Author: David Low and Sebastian Brown
 * Modified: 2 May 2019
 * Purpose: Provide a 10-round AES cryptographic system for COMP3260.
 * Notes: The class provides four public methods that may be called.
 */

/**
 * An enum used to define directions for shifting rows.
 */
enum Direction {
    LEFT, RIGHT
}

/**
 * The AES abstract class provides all the methods needed to implement both
 * encryption and decryption in AES. The flexibility in implementing the actual
 * encryption/decryption logic allows developers the chance to tweak the AES
 * logic and analyse various effects such as the Avalanche Effect.
 */
public abstract class AES {
    protected static final int NUM_OF_ROUNDS = 10;

    protected static final int[] S_BOX = {  };

    protected static final int[] INV_S_BOX = {  };

    protected static final int[] MUL_2 = {  };

    protected static final int[] MUL_3 = {  };

    protected static final int[] MUL_9 = {  };

    protected static final int[] MUL_11 = {  };

    protected static final int[] MUL_13 = {  };

    protected static final int[] MUL_14 = {  };

    protected static final int[] R_CON = { 0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, };

    /**
     * The encrypt(String, String) method allows a developer to enter two binary
     * strings instead of two byte arrays. It will return an array of binary
     * strings, one for each round in the AES algorithm.
     * 
     * @param plaintext the plaintext block that is to be encrypted.
     * @param key       the cipher key that is to be used to encrypt the plaintext
     *                  block.
     * @return an array of each result at the end of a round.
     */
    public String[] encrypt(String plaintext, String key) {
        String[] stringResults = new String[NUM_OF_ROUNDS];

        byte[] bytePlainText = stringToByteArray(plaintext);
        byte[] byteKey = stringToByteArray(key);

        byte[][] byteResults = encrypt(bytePlainText, byteKey);

        for (int i = 0; i < NUM_OF_ROUNDS; i++) {
            stringResults[i] = byteArrayToString(byteResults[i]);
        }
        return stringResults;
    }

    /**
     * The decrypt(String, String) method allows a developer to enter two binary
     * strings instead of two byte arrays. It will return an array of binary
     * strings, one for each round in the AES algorithm.
     * 
     * @param plaintext the plaintext block that is to be decrypted.
     * @param key       the cipher key that is to be used to decrypt the plaintext
     *                  block.
     * @return an array of each result at the end of a round.
     */
    public String[] decrypt(String ciphertext, String key) {
        String[] stringResults = new String[NUM_OF_ROUNDS];

        byte[] byteCipherText = stringToByteArray(ciphertext);
        byte[] byteKey = stringToByteArray(key);

        byte[][] byteResults = decrypt(byteCipherText, byteKey);

        for (int i = 0; i < NUM_OF_ROUNDS; i++) {
            stringResults[i] = byteArrayToString(byteResults[i]);
        }
        return stringResults;
    }

    /**
     * Accepts 16-byte text and key arrays, encrypts them according to AES and
     * returns all intermediary and final results.
     * 
     * @param plaintext the plaintext block that is to be encrypted.
     * @param key       the cipher key that is to be used to encrypt the plaintext
     *                  block.
     * @return an array of each result at the end of a round.
     */
    public abstract byte[][] encrypt(byte[] plaintext, byte[] key);

    /**
     * Accepts 16-byte text and key arrays, decrypts them according to AES and
     * returns all intermediary and final results.
     * 
     * @param plaintext the plaintext block that is to be decrypted.
     * @param key       the cipher key that is to be used to decrypt the plaintext
     *                  block.
     * @return an array of each result at the end of a round.
     */
    public abstract byte[][] decrypt(byte[] ciphertext, byte[] key);

    /**
     * Substitutes sixteen bytes in a byte array through a substitution box.
     */
    protected byte[] substituteBytes(byte[] text, int[] subBox) {
        // System.out.println("Text Before Substituting: " + stringifyByteArray(text));
        byte[] substitutedText = new byte[16];
        for (int i = 0; i < 16; i++) {
            substitutedText[i] = (byte) subBox[text[i] & 0xFF];
        }
        // System.out.println("Text After Substituting: " +
        // stringifyByteArray(substitutedText));
        return substitutedText;
    }

    /**
     * Shifts four rows in a 4x4 grid (stored as a 16-element 1D array)
     */
    protected byte[] shiftRows(byte[] text, Direction direction) {
        // System.out.println("Text Before shifting: " + stringifyByteArray(text));
        byte[] result = new byte[16];
        for (int i = 0; i < 4; i++) {
            switch (direction) {
            case LEFT:
                result[i] = text[(i + (i * 4)) % 16];
                result[i + 4] = text[(i + 4 + (i * 4)) % 16];
                result[i + 8] = text[(i + 8 + (i * 4)) % 16];
                result[i + 12] = text[(i + 12 + (i * 4)) % 16];
                break;
            case RIGHT:
                result[i] = text[((i - (i * 4)) + 16) % 16];
                result[i + 4] = text[((i + 4 - (i * 4)) + 16) % 16];
                result[i + 8] = text[((i + 8 - (i * 4)) + 16) % 16];
                result[i + 12] = text[((i + 12 - (i * 4)) + 16) % 16];
                break;
            }
        }
        // System.out.println("Text After shifting: " + stringifyByteArray(result));
        return result;
    }

    /**
     * Mixes columns in a 4x4 grid (stored as a 16-element 1D array)
     */
    protected byte[] mixColumns(byte[] text) {
        // System.out.println("Text Before Mixing: " + stringifyByteArray(text));

        byte[] mixedText = new byte[16];

        for (int i = 0; i < 4; i++) {
            mixedText[4 * i + 0] = (byte) (MUL_2[text[(4 * i) + 0] & 0xFF] ^ MUL_3[text[(4 * i) + 1] & 0xFF]
                    ^ text[(4 * i) + 2] ^ text[(4 * i) + 3]);
            mixedText[4 * i + 1] = (byte) (text[(4 * i) + 0] ^ MUL_2[text[(4 * i) + 1] & 0xFF]
                    ^ MUL_3[text[(4 * i) + 2] & 0xFF] ^ text[(4 * i) + 3]);
            mixedText[4 * i + 2] = (byte) (text[(4 * i) + 0] ^ text[(4 * i) + 1] ^ MUL_2[text[(4 * i) + 2] & 0xFF]
                    ^ MUL_3[text[(4 * i) + 3] & 0xFF]);
            mixedText[4 * i + 3] = (byte) (MUL_3[text[(4 * i) + 0] & 0xFF] ^ text[(4 * i) + 1] ^ text[(4 * i) + 2]
                    ^ MUL_2[text[(4 * i) + 3] & 0xFF]);

        }

        // System.out.println("Text After Mixing: " + stringifyByteArray(mixedText));
        return mixedText;
    }

    /**
     * Inverts mixed columns in a 4x4 grid (stored as a 16-element 1D array)
     */
    protected byte[] invertMixedColumns(byte[] text) {
        // System.out.println("Text Before Inverse Mixing: " +
        // stringifyByteArray(text));
        byte[] invertedText = new byte[16];

        for (int i = 0; i < 4; i++) {
            invertedText[4 * i + 0] = (byte) (MUL_14[text[(4 * i) + 0] & 0xFF] ^ MUL_11[text[(4 * i) + 1] & 0xFF]
                    ^ MUL_13[text[(4 * i) + 2] & 0xFF] ^ MUL_9[text[(4 * i) + 3] & 0xFF]);
            invertedText[4 * i + 1] = (byte) (MUL_9[text[(4 * i) + 0] & 0xFF] ^ MUL_14[text[(4 * i) + 1] & 0xFF]
                    ^ MUL_11[text[(4 * i) + 2] & 0xFF] ^ MUL_13[text[(4 * i) + 3] & 0xFF]);
            invertedText[4 * i + 2] = (byte) (MUL_13[text[(4 * i) + 0] & 0xFF] ^ MUL_9[text[(4 * i) + 1] & 0xFF]
                    ^ MUL_14[text[(4 * i) + 2] & 0xFF] ^ MUL_11[text[(4 * i) + 3] & 0xFF]);
            invertedText[4 * i + 3] = (byte) (MUL_11[text[(4 * i) + 0] & 0xFF] ^ MUL_13[text[(4 * i) + 1] & 0xFF]
                    ^ MUL_9[text[(4 * i) + 2] & 0xFF] ^ MUL_14[text[(4 * i) + 3] & 0xFF]);

        }
        // System.out.println("Text After Inverse Mixing: " +
        // stringifyByteArray(invertedText));
        return invertedText;
    }

    /**
     * Generates all subkeys based on the Rjindael key structure.
     */
    protected byte[][] generateKeys(byte[] masterKey) {
        byte[] expandedKey = new byte[16 * (NUM_OF_ROUNDS + 1)];

        // Copies master key into first subkey.
        for (int i = 0; i < 16; i++) {
            expandedKey[i] = masterKey[i];
        }

        for (int round = 1; round <= NUM_OF_ROUNDS; round++) {
            // For each word
            byte[] prevWord = new byte[4];
            for (int i = 0; i < 4; i++) {
                // Get Previous word
                for (int j = 0; j < 4; j++) {
                    prevWord[j] = expandedKey[round * 16 + i * 4 + j - 4];
                }

                if (i == 0) {
                    prevWord = modKeyWord(prevWord, round);
                }

                for (int j = 0; j < 4; j++) {
                    expandedKey[(round) * 16 + i * 4
                            + j] = (byte) (expandedKey[(round - 1) * 16 + i * 4 + j] ^ prevWord[j]);
                }
            }
        }

        byte[][] subKeys = new byte[16][NUM_OF_ROUNDS + 1];
        for (int i = 0; i < NUM_OF_ROUNDS + 1; i++) {
            byte[] subKey = new byte[16];
            for (int j = 0; j < 16; j++) {
                subKey[j] = expandedKey[i * 16 + j];
            }
            subKeys[i] = subKey;
        }

        return subKeys;
    }

    /**
     * Modifies the key word for the key generation schedule
     */
    protected byte[] modKeyWord(byte[] word, int counter) {
        byte[] modWord = new byte[4];

        // Rotate word
        modWord[0] = word[1];
        modWord[1] = word[2];
        modWord[2] = word[3];
        modWord[3] = word[0];

        // Substitute word
        for (int i = 0; i < 4; i++) {
            modWord[i] = (byte) S_BOX[modWord[i] & 0xFF];
        }

        // RCON
        modWord[0] = (byte) (modWord[0] ^ R_CON[counter]);
        return modWord;
    }

    /**
     * XORs the text with the key.
     */
    protected byte[] addRoundKey(byte[] text, byte[] key) {
        byte[] xoredText = new byte[16];
        for (int i = 0; i < 16; i++) {
            xoredText[i] = (byte) (text[i] ^ key[i]);
        }
        return xoredText;
    }

    /**
     * Converts a boolean string into a byte array
     */
    protected byte[] stringToByteArray(String input) {
        byte[] output = new byte[16];

        for (int i = 0; i < 16; i++) {
            output[i] = (byte) Integer.parseInt(input.substring(i * 8, i * 8 + 8), 2);
        }

        return output;
    }

    /**
     * Converts a byte array into a representational boolean string
     */
    protected String byteArrayToString(byte[] input) {
        String output = "";
        for (int i = 0; i < 16; i++) {
            output += String.format("%8s", Integer.toBinaryString(input[i] & 0xFF)).replace(' ', '0');
            // hexText += String.format("%02X", byteResults[i][j]);
        }
        return output;
    }

    /**
     * Creates a hex string value of any byte array
     */
    // private String stringifyByteArray(byte[] input) {
    // String output = "";
    // for (int i = 0; i < input.length; i++) {
    // output += String.format("%02X", input[i]).toLowerCase();
    // }
    // return output;
    // }
}