enum Direction {
    LEFT, RIGHT
}

public class AES {
    protected static int numOfRounds = 10;

    protected static int[] SBOX = {  };

    protected static int[] INV_SBOX = {  };

    protected static int[] MUL_2 = {  };

    protected static int[] MUL_3 = {  };

    protected static int[] MUL_9 = {  };

    protected static int[] MUL_11 = {  };

    protected static int[] MUL_13 = {  };

    protected static int[] MUL_14 = {  };

    protected static int[] R_CON = { 0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, };

    public AES() {

    }

    // Provides string to byte conversion and back for encryption.
    public String[] encrypt(String plaintext, String key) {
        byte[] bytePlainText = stringToByteArray(plaintext);
        byte[] byteKey = stringToByteArray(key);

        byte[][] byteResults = encrypt(bytePlainText, byteKey);
        String[] stringResults = new String[numOfRounds + 1];

        for (int i = 0; i < numOfRounds; i++) {
            stringResults[i] = byteArrayToString(byteResults[i]);
        }
        return stringResults;
    }

    // Accepts 16-byte text and key arrays, encrypts them according to AES and
    // returns all intermediary results.
    public byte[][] encrypt(byte[] plaintext, byte[] key) {
        byte[][] subKeys = generateKeys(key);

        byte[][] results = new byte[16][10];

        // Initial Key XOR
        byte[] result = addRoundKey(plaintext, subKeys[0]);

        // 9 rounds
        for (int i = 1; i <= 9; i++) {
            result = substituteBytes(result, SBOX);
            result = shiftRows(result, Direction.LEFT);
            result = mixColumns(result);
            result = addRoundKey(result, subKeys[i]);
            results[i - 1] = result;
        }

        System.out.println(results.length);
        // Final round
        result = substituteBytes(result, SBOX);
        result = shiftRows(result, Direction.LEFT);
        result = addRoundKey(result, subKeys[10]);
        System.out.println("Adding final results");

        results[9] = result;
        for (int i = 0; i < 10; i++) {
            System.out.println(stringifyByteArray(results[i]));
        }

        return results;
    }

    // public decrypt() {

    // }

    protected byte[] substituteBytes(byte[] text, int[] subBox) {
        // convert text byte into hex representation
        System.out.println("Substituting bytes");
        byte[] substitutedText = new byte[16];

        for (int i = 0; i < 16; i++) {
            // System.out.println("Hexbyte: " + hexByte);
            // int rowIndex = Integer.parseInt(hexByte.substring(0, 1), 16) * 16;
            // int colIndex = Integer.parseInt(hexByte.substring(1, 2), 16);
            // substitutedText[i] = (byte) subBox[rowIndex + colIndex];
            // System.out.println("Hexbyte: " + (text[i] & 0xFF));
            substitutedText[i] = (byte) subBox[text[i] & 0xFF];
            // System.out.println(String.format("%02X", text[i]).toLowerCase() + " > "
            // + String.format("%02X", substitutedText[i]).toLowerCase());
        }

        return substitutedText;
    }

    protected byte[] shiftRows(byte[] text, Direction direction) {
        byte[] result = new byte[16];
        for (int i = 0; i < 4; i++) {
            switch (direction) {
            case LEFT:
                result[i] = text[(i + (i * 4)) % 16];
                result[i + 4] = text[(i + 4 + (i * 4)) % 16];
                result[i + 8] = text[(i + 8 + (i * 4)) % 16];
                result[i + 12] = text[(i + 12 + (i * 4)) % 16];
                break;
            case RIGHT:
                result[i] = text[((i - (i * 4)) + 16) % 16];
                result[i + 4] = text[((i + 4 - (i * 4)) + 16) % 16];
                result[i + 8] = text[((i + 8 - (i * 4)) + 16) % 16];
                result[i + 12] = text[((i + 12 - (i * 4)) + 16) % 16];
                break;
            }
        }
        return result;
    }

    protected byte[] mixColumns(byte[] text) {
        byte[] mixedText = new byte[16];

        System.out.println("Pre-Mixed text: " + stringifyByteArray(text));
        for (int i = 0; i < 4; i++) {
            mixedText[4 * i + 0] = (byte) (MUL_2[text[(4 * i) + 0] & 0xFF] ^ MUL_3[text[(4 * i) + 1] & 0xFF]
                    ^ text[(4 * i) + 2] ^ text[(4 * i) + 3]);
            mixedText[4 * i + 1] = (byte) (text[(4 * i) + 0] ^ MUL_2[text[(4 * i) + 1] & 0xFF]
                    ^ MUL_3[text[(4 * i) + 2] & 0xFF] ^ text[(4 * i) + 3]);
            mixedText[4 * i + 2] = (byte) (text[(4 * i) + 0] ^ text[(4 * i) + 1] ^ MUL_2[text[(4 * i) + 2] & 0xFF]
                    ^ MUL_3[text[(4 * i) + 3] & 0xFF]);
            mixedText[4 * i + 3] = (byte) (MUL_3[text[(4 * i) + 0] & 0xFF] ^ text[(4 * i) + 1] ^ text[(4 * i) + 2]
                    ^ MUL_2[text[(4 * i) + 3] & 0xFF]);

        }
        System.out.println("Post-Mixed text: " + stringifyByteArray(mixedText));
        return mixedText;
    }

    protected byte[] invertMixedColumns(byte[] text) {
        byte[] invertedText = new byte[16];

        // System.out.println("Mixed text: " + stringifyByteArray(text));
        for (int i = 0; i < 4; i++) {
            invertedText[4 * i + 0] = (byte) (MUL_14[text[(4 * i) + 0] & 0xFF] ^ MUL_11[text[(4 * i) + 1] & 0xFF]
                    ^ MUL_13[text[(4 * i) + 2] & 0xFF] ^ MUL_9[text[(4 * i) + 3] & 0xFF]);
            invertedText[4 * i + 0] = (byte) (MUL_9[text[(4 * i) + 0] & 0xFF] ^ MUL_14[text[(4 * i) + 1] & 0xFF]
                    ^ MUL_11[text[(4 * i) + 2] & 0xFF] ^ MUL_13[text[(4 * i) + 3] & 0xFF]);
            invertedText[4 * i + 0] = (byte) (MUL_13[text[(4 * i) + 0] & 0xFF] ^ MUL_9[text[(4 * i) + 1] & 0xFF]
                    ^ MUL_14[text[(4 * i) + 2] & 0xFF] ^ MUL_11[text[(4 * i) + 3] & 0xFF]);
            invertedText[4 * i + 0] = (byte) (MUL_11[text[(4 * i) + 0] & 0xFF] ^ MUL_13[text[(4 * i) + 1] & 0xFF]
                    ^ MUL_9[text[(4 * i) + 2] & 0xFF] ^ MUL_14[text[(4 * i) + 3] & 0xFF]);

        }
        // System.out.println("Inverted text: " + stringifyByteArray(invertedText));
        return invertedText;
    }

    protected byte[][] generateKeys(byte[] masterKey) {
        // for (int i = 0; i < 16; i++) {
        // masterKey[i] = (byte) (0x00 + i);
        // }
        System.out.println("Master key is: " + stringifyByteArray(masterKey));
        byte[] expandedKey = new byte[176];

        for (int i = 0; i < 16; i++) {
            expandedKey[i] = masterKey[i];
        }

        for (int round = 1; round <= 10; round++) {
            // For each word
            byte[] prevWord = new byte[4];
            for (int i = 0; i < 4; i++) {
                // Get Previous word
                for (int j = 0; j < 4; j++) {
                    prevWord[j] = expandedKey[round * 16 + i * 4 + j - 4];
                }

                if (i == 0) {
                    prevWord = modKeyWord(prevWord, round);
                }

                for (int j = 0; j < 4; j++) {
                    expandedKey[(round) * 16 + i * 4
                            + j] = (byte) (expandedKey[(round - 1) * 16 + i * 4 + j] ^ prevWord[j]);
                }
            }
        }

        byte[][] subKeys = new byte[16][11];
        for (int i = 0; i < 11; i++) {
            byte[] subKey = new byte[16];
            for (int j = 0; j < 16; j++) {
                subKey[j] = expandedKey[i * 16 + j];
            }
            subKeys[i] = subKey;
        }

        System.out.println("Sub keys are as follows:");
        for (int i = 0; i < 11; i++) {
            System.out.println(stringifyByteArray(subKeys[i]));
        }

        return subKeys;
    }

    protected byte[] modKeyWord(byte[] word, int counter) {
        byte[] modWord = new byte[4];

        // Rotate
        modWord[0] = word[1];
        modWord[1] = word[2];
        modWord[2] = word[3];
        modWord[3] = word[0];

        // Substitute
        for (int i = 0; i < 4; i++) {
            modWord[i] = (byte) SBOX[modWord[i] & 0xFF];
        }

        // RCON
        modWord[0] = (byte) (modWord[0] ^ R_CON[counter]);
        return modWord;
    }

    protected byte[] addRoundKey(byte[] text, byte[] key) {
        // System.out.println("Incoming text: " + stringifyByteArray(text));
        // System.out.println("Incoming key: " + stringifyByteArray(key));
        byte[] xoredText = new byte[16];
        for (int i = 0; i < 16; i++) {
            xoredText[i] = (byte) (text[i] ^ key[i]);
        }
        // System.out.println("Outgoing text: " + stringifyByteArray(xoredText));
        return xoredText;
    }

    protected byte[] stringToByteArray(String input) {
        byte[] output = new byte[16];

        for (int i = 0; i < 16; i++) {
            output[i] = (byte) Integer.parseInt(input.substring(i * 8, i * 8 + 8), 2);
        }

        return output;
    }

    protected String byteArrayToString(byte[] input) {
        String output = "";
        for (int i = 0; i < 16; i++) {
            output += String.format("%8s", Integer.toBinaryString(input[i] & 0xFF)).replace(' ', '0');
            // hexText += String.format("%02X", byteResults[i][j]);
        }
        return output;
    }

    private String stringifyByteArray(byte[] input) {
        String output = "";
        for (int i = 0; i < input.length; i++) {
            output += String.format("%02X", input[i]).toLowerCase();
        }
        return output;
    }
}